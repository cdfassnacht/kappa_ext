...
""" 
This file defines transformations that are used in the Cosmap
analysis for computing weighted number counts in the sky survey data.

There are two classes of transformations. Setup transformations run a
single time at the beginning of the analysis to (as the name suggest)
do any necessary setup. Main transformations are run for each sample
region and are responsible for doing the actual
computation. Additional information about the transformations can be
found in transformations.json, which defines the order for running
transformations, types of data required, and any required input
parameters.
...
def build_filter(z_s, inner_radius, radius, limiting_magnitude,
                 column_aliases) -> Callable:
...
class Setup:
    ...
    def build_filter_sets(
        ...
        filters = {}
        """
        Create keys for the filter sets by combining the radius and 
        limiting magnitude
        """
        lens_name = lens_parameters.lens_name
        lens_data = lens_parameters.lens_data
        for r in radius:
            for m in limiting_magnitude:
                ...
                filter = build_filter(
                    lens_parameters.source_redshift, inner_radius, r, m,
                    column_aliases
                )
                filters[key] = filter
        return filters
    ...
    def build_output_paths(filter_sets, base_output_path) -> dict:
        """
        Each filter corresponds to its own output file (i.e. mag < 24, 120"). 
        This function builds the paths based on the output folder provided in 
        the run parameters and the filter keys.

        The output paths are returned as a dictionary. The keys of that 
        dictionary will be used throughout the analysis to reference a 
        specific setup.
        """
    ...
    @staticmethod
    def get_lens_data(lens_parameters):
        lens_data = {}
        indir = lens_parameters.data_path
        catname = path.join(indir, 'catalog.txt')
        tmpcat = ascii.read(catname, format='commented_header')
        skycoord = SkyCoord(tmpcat['ra'], tmpcat['dec'], unit=u.deg)
        tmpcat['coordinates'] = skycoord
        tmpcat['radius'] = lens_parameters.lens_coordinate.separation(skycoord)
        lens_data['catalog'] = tmpcat
        """
        Mask must have WCS information so that it can be applied to the
        catalog data
        """
        maskname = path.join(indir, 'mask.fits')
        lensmask = pf.open(maskname)
        lens_data['mask'] = Mask([lensmask], mask_key=0)
        return lens_data


class Main:
    @staticmethod
    def apply_masks(catalog, mask, lens_parameters, sample_region, *args,
                    **kwargs):
        """
        In order to directly compare the lens field to a field in the
        reference survey we need to make sure one is not masked more
        than the other. To do this, we apply both masks to both
        catalogs before computing counts.
        ...
        lens_coordinate = lens_parameters["lens_coordinate"]
        lens_catalog = lens_parameters['lens_data']["catalog"]
        lens_mask = lens_parameters['lens_data']["mask"]
    ...
    def apply_filters(
        ...
        """
        Create several different copies of the final catalogs, each with a 
        different
    ...
    @staticmethod
    def count(catalogs, sample_region, dataset_name, lens_parameters, *args,
              **kwargs):
        
        """

        Compute the actual counts. The input to this function
        (catalogs) is the output of the apply_filters function.

        This function is marked as the output transformation in the
        transformations.json file, so Cosmap understands that it
        should treat the ouput of this function as the result for this
        iteration.

        The keys of the ouput dictionary correspond to the same keys
        that were produced by the build_output_paths function in the
        setup transformations. Cosmap will automatically collate the
        results on a per-file basis and periodically write them to
        disk.
        ...
        for lens_name, lens_catalogs_ in catalogs.items():
            z_source = lens_parameters["source_redshift"]
